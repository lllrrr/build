...
local config_file = "/etc/config/qbittorrent" % config_dir
local session_file = "%s/qBittorrent/config/qBittorrent.conf" % config_dir
...
s = m:section(SimpleSection, nil, translatef("这是<code>%s</code>下的配置文件内容：", config_file))
...
s = m:section(SimpleSection, nil, translatef("这是<code>%s</code>下的配置文件内容：", session_file))
